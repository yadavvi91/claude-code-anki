...
5. Include before/after metrics in the PR body`} /></Reveal>
        ...
        <Reveal><p style={styles.prose}><strong>In VS Code</strong>, you get two built-in review modes. <em>Selection review</em>: highlight code, right-click, and choose Review for targeted feedback on a specific block. <em>Uncommitted changes review</em>: click the Copilot Code Review button in the Source Control panel to review everything you've changed before committing. Feedback appears as inline editor comments you can accept or dismiss. This is powerful for <strong>pre-commit self-review</strong> — catching issues before code even leaves your machine.</p></Reveal>
        <Reveal><h3 style={styles.h3}>Can you do PR review inside VS Code?</h3></Reveal>
        <Reveal><p style={styles.prose}>The automated "add Copilot as reviewer → get inline PR comments" experience is <strong>github.com-only</strong>. There is no button in VS Code that triggers the same PR-level review you see on the web. However, there are practical workarounds that keep you in the editor:</p></Reveal>
        <Reveal><ComparisonTable headers={['Workaround', 'How']} rows={[
          ['GitHub Pull Requests extension', 'Install the extension, check out a PR locally, then use Copilot\'s selection review on the diff views — right-click any changed file and review it block by block'],
          ['Copilot Chat + git diff', 'In Agent Mode, ask Copilot to run git diff and review the output — it can analyze the full diff and give feedback inline in the chat'],
          ['GitHub CLI from terminal', 'Run gh pr edit --add-reviewer copilot in VS Code\'s integrated terminal — triggers the github.com review without leaving the editor, comments appear on the PR'],
        ]} /></Reveal>
        <Reveal><p style={styles.prose}>The GitHub Pull Requests extension is the closest to a native in-editor PR review experience. It renders PR diffs directly in VS Code, so you can apply Copilot's selection-based code review to each changed file. It's not automatic like the web version, but it keeps your entire review cycle inside the editor.</p></Reveal>
        ...
        <Reveal><TipCallout variant="warning">Full PR-level Copilot review (the "add as reviewer" flow that leaves comments on the PR) is github.com-only. In VS Code, you can review selections and uncommitted changes natively, or use the GitHub Pull Requests extension + Copilot Chat as a workaround. JetBrains and other IDEs don't have local Copilot code review yet — only VS Code and Xcode.</TipCallout></Reveal>
...
const styles = {
  ...
  h3: { fontFamily: T.font.heading, fontSize: '1.25rem', fontWeight: 600, color: T.color.ink2, lineHeight: 1.3, marginBottom: '1rem', marginTop: '0.5rem' },
